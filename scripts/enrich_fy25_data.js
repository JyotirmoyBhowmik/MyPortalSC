...
async function main() {
  console.log('🚀 Generating enrich_fy25.sql...');
  let sql = '';

  // ─── 1. Reset all initiatives image_url to NULL ───
  console.log('📦 Resetting initiatives.image_url to NULL (initiatives do not display images)...');
  const { error: resetErr } = await supabase
    .from('initiatives')
    .update({ image_url: null })
    .neq('id', '00000000-0000-0000-0000-000000000000');
  if (resetErr) {
    console.error('Error resetting initiatives.image_url:', resetErr);
  } else {
    console.log('✅ Initiatives image_url successfully cleared.');
  }

  // ─── 2. Enrich 10 FY 2025-26 Flagship Projects ───
  console.log('🏢 Enriching 10 FY 2025-26 Flagship Projects...');
...
Configured intelligent SD-WAN path-selection and Quality of Service (QoS) traffic prioritization policies, guaranteeing dedicated low-latency bandwidth for mission-critical SAP ERP transactions and enterprise VoIP while routing general web traffic over secondary commodity links.`,
  ...
  for (const p of projectsToUpdate) {
    const { error } = await supabase
      .from('projects')
      .update({
        delivery_focus: p.delivery_focus,
    ...
    if (error) {
      console.error(`Error updating project ${p.slug}:`, error);
    } else {
      console.log(`  ✓ Updated project: ${p.slug}`);
    }
  }

  // ─── 3. Enrich 20 FY 2025-26 Initiatives ───
  console.log('📋 Enriching 20 FY 2025-26 Initiatives (description & business_value)...');
  ...
  for (const init of initiativesToUpdate) {
    ...
    if (error) {
      console.error(`Error updating initiative ${init.slug}:`, error);
    } else {
      console.log(`  ✓ Updated initiative: ${init.slug}`);
    }
  }

  // ─── 4. Set fallback description & business_value for any earlier initiatives ───
  console.log('🔄 Setting contextual fallback description & business_value for earlier initiatives...');
  const { data: earlyInits, error: fetchErr } = await supabase
    .from('initiatives')
    .select('id, title, strategic_area, delivery_focus, description, business_value')
    .or('description.is.null,business_value.is.null');

  if (fetchErr) {
    console.error('Error fetching earlier initiatives:', fetchErr);
  } else if (earlyInits && earlyInits.length > 0) {
    console.log(`Found ${earlyInits.length} initiatives needing description or business_value. Updating...`);
    for (const item of earlyInits) {
      const updates = {};
      if (!item.description) {
        updates.description = `Delivered under the ${item.strategic_area} strategic program, this initiative focused on ${item.title.toLowerCase()} to elevate enterprise IT maturity, service reliability, and operational standardization.`;
      }
      if (!item.business_value) {
        updates.business_value = `Strengthened organizational service availability, mitigated infrastructure downtime risks, and ensured full adherence to corporate IT governance standards.`;
      }
      if (Object.keys(updates).length > 0) {
        updates.image_url = null;
        updates.updated_at = new Date().toISOString();
        await supabase.from('initiatives').update(updates).eq('id', item.id);
      }
    }
    console.log('✅ Earlier initiatives enriched with contextual descriptions and business values.');
  }

  // ─── 5. Update content_pages for Home & About ───
  console.log('📄 Updating content_pages (Home & About) in Supabase...');
  ...
  if (aboutData && aboutData.content) {
    const updatedContent = { ...aboutData.content };
    if (Array.isArray(updatedContent.experience) && updatedContent.experience.length > 0) {
      updatedContent.experience[0].role = 'Manager – IT Infrastructure & Network (Grade IS4)';
      updatedContent.experience[0].description = 'Lead infrastructure operations and project delivery across Data Center, Disaster Recovery, Industrial OT/ICS, and SAP ERP environments; manage vendors, budgets, and global project teams, delivering 108+ enterprise initiatives across corporate headquarters and multi-plant manufacturing operations.';
    }
    await supabase
      .from('content_pages')
      .update({ content: updatedContent, updated_at: new Date().toISOString() })
      .eq('page_key', 'about');
    console.log('✅ Updated content_pages: about');
  }
...
main().catch(err => {
  console.error('Fatal error during data enrichment:', err);
  process.exit(1);
});
