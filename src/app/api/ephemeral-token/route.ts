        ...
        const token = await ai.authTokens.create({
            config: {
                ...
                liveConnectConstraints: {
                    model: MODEL,
                    config: {
                        responseModalities: ["AUDIO"] as unknown as any, // eslint-disable-line @typescript-eslint/no-explicit-any
                        temperature: 0.7,
                    }
                },
...
`;
        ...
        console.error('Ephemeral token error:', error instanceof Error ? error.message : error);
